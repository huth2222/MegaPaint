...
function GetEditAdminTable(admin_code,fullname){
    //alert('Hi');
    $('#div_index').hide();
    GetMenuEditAdmin();
    GetPageEditAdmin("/Admin/EditAdminProfile?account_code="+  admin_code,admin_code);
    $('#edit_content').show();
        
        // $.ajax({
        //     url: "/Admin/EditAdminProfile?account_code="+  admin_code,
        //     type: 'GET',
...
/************* Get Account table */
function GetAdminTable(active,admin_code) {
    //alert(admin_code);
    if(admin_code == null || admin_code == ""){
        admin_code = "empty";
    }
    let local = location.origin + "/Admin/GetAdminTable/" + active + '/' + admin_code;
    $.get(local,
        function (data, textStatus, jqXHR) {
            ...
            $.each(data, function (indexInArray, valueOfElement) {
                fullname = valueOfElement.prefix_name_th + valueOfElement.firstname + ' ' + valueOfElement.lastname;
                //adminCode = valueOfElement.admin_code.padStart(8, "0");
                if(valueOfElement.avatar == '' || valueOfElement.avatar == null){
                    img = "";
                }else{
                    img = "<img id='img0' alt='' width='80' src='/data/admin/profile/"+valueOfElement.avatar+"' style='cursor: pointer;height:80px;object-fit:cover;'>";
                }
                ...
                if($('#account').val() == valueOfElement.admin_code){
                ...
                html += tr +
                    ...
                    '<td ' + h_age + '>' + age + '</td>' +                    
                    '<td><span class="d-flex justify-content-around"> <span class="hover-del" onclick="GetDelAdminTable(' + "'" + valueOfElement.admin_code + "'," + "'" + fullname + "'" + ')"><i class="fas fa-trash-alt"></i></span><span class="hover-edit" onclick="GetEditAdminTable(' + "'" + valueOfElement.admin_code + "'," + "'" + fullname + "'" + ')"><i class="fas fa-edit"></i></span> </span></td>' +                  
                '</tr>';
                n++;
            });
            html += '</tbody></table>';
            $('#div_addTable').html(html);
            if(focus != ''){
                $('#idfocus').focus();
            }
        },
...
/************** For delete account */
function GetDelAdminTable(admin_code,fullname){
    let msg = 'ยืนยันการลบข้อมูล ' + fullname;
    if(confirm(msg)){
        GetAdminTable("del",admin_code);
    }    
}
...
function GetSubmit(form) {
  ...
  let = active = "";
  if(form == "FormAdd"){
    formAction = "/Admin/AddAdminProfile";
    active = "add";
  }else if(form == "FormEdit"){
    formAction = "/Admin/EditAdminProfile";
    active = "edit";
  }
  setTimeout(() => {
    $.ajax({
